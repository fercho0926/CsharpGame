...
export async function sendToGoogleSheets(endpoint:string,event:SyncEvent){
 ...
 try{await fetch(endpoint.trim(),{method:"POST",mode:"no-cors",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(event),keepalive:true});return true}catch{return false}
}
