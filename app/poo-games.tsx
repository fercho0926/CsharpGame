...
const pillars: Pillar[] = [
  {
    ...
    challenges: [
      ...
      { prompt: "¿Qué modificador permite acceso a la clase y a sus clases derivadas?", code: "___ int score;", options: ["protected", "private", "internal", "public"], answer: "protected", explanation: "protected permite que la clase que declara el miembro y sus clases derivadas lo usen. No significa que todo el proyecto pueda acceder." },
      ...
      { prompt: "¿Qué modificador usarías para una clase auxiliar visible solo dentro del mismo proyecto?", code: "___ class TaxCalculator { }", options: ["internal", "protected", "private", "public"], answer: "internal", explanation: "internal limita el acceso al mismo assembly, que normalmente corresponde al proyecto compilado. Es la palabra que suele olvidarse." },
      { prompt: "¿Qué ocurre con un miembro `private` cuando una clase hereda de otra?", code: "class Child : Parent { }", options: ["Child no puede acceder directamente a ese miembro", "Child lo recibe como public", "Child lo recibe siempre como protected", "El miembro se vuelve internal"], answer: "Child no puede acceder directamente a ese miembro", explanation: "private pertenece exclusivamente al tipo que lo declara. Si una hija necesita usarlo, la base puede exponer métodos o elegir protected con cuidado." },
      { prompt: "¿Qué diferencia clave hay entre `protected` e `internal`?", code: "protected int A;\ninternal int B;", options: ["protected se basa en herencia; internal en pertenecer al mismo assembly", "Ambos significan exactamente lo mismo", "protected es para todo el proyecto; internal solo para la clase", "internal exige herencia y protected no"], answer: "protected se basa en herencia; internal en pertenecer al mismo assembly", explanation: "protected mira la relación entre tipos; internal mira si el código está compilado en el mismo assembly/proyecto." },
      { prompt: "¿Qué significa `protected internal`?", code: "protected internal void Reset() { }", options: ["Acceso desde el mismo assembly o desde una clase derivada", "Acceso solo desde la clase declaradora", "Acceso únicamente desde otro assembly sin herencia", "Acceso público para cualquier aplicación"], answer: "Acceso desde el mismo assembly o desde una clase derivada", explanation: "La combinación usa una regla OR: basta pertenecer al mismo assembly o ser una clase derivada." },
      { prompt: "¿Qué significa `private protected`?", code: "private protected int Id;", options: ["Solo la clase y sus derivadas dentro del mismo assembly", "Cualquier clase derivada de cualquier assembly", "Todo el assembly sin necesidad de herencia", "Solo métodos static"], answer: "Solo la clase y sus derivadas dentro del mismo assembly", explanation: "Es la combinación más restrictiva entre herencia y assembly: exige ambas condiciones, es decir, una regla AND." },
      { prompt: "Una API no debe permitir que consumidores externos cambien `balance`. ¿Cuál diseño comunica mejor esa regla?", code: "public decimal Balance { get; ___ set; }", options: ["private", "public", "protected internal", "internal"], answer: "private", explanation: "Un setter private permite consultar desde fuera, pero solo la propia clase puede cambiar el valor mediante reglas controladas." },
      { prompt: "¿Cuál modificador es más amplio dentro del proyecto que `private`?", code: "___ class AuditLog { }", options: ["internal", "private protected", "protected", "private"], answer: "internal", explanation: "internal permite acceso a cualquier código del mismo assembly; private se limita al tipo que declara el miembro." },
      { prompt: "¿Por qué no debes elegir `public` para todos los campos?", code: "public decimal balance;", options: ["Permitiría cambios sin validación y rompería invariantes", "Porque public solo funciona con strings", "Porque public impide usar métodos", "Porque public convierte el campo en static"], answer: "Permitiría cambios sin validación y rompería invariantes", explanation: "La encapsulación expone una superficie pequeña y controla las operaciones que pueden dejar el objeto en un estado válido." },
      { prompt: "¿Qué regla de memoria ayuda a recordar `private`, `protected`, `internal` y `public`?", code: "private < protected/internal < public", options: ["Privado: clase; protected: herencia; internal: proyecto; public: todos", "Privado: proyecto; protected: todos; internal: herencia; public: nadie", "Todos tienen el mismo alcance", "Solo public y private son modificadores reales"], answer: "Privado: clase; protected: herencia; internal: proyecto; public: todos", explanation: "Piensa primero en quién necesita usar el miembro y concede el alcance mínimo que permita el diseño." },
    ],
  },
  {
    id: "abstraccion",
...
export function GamesHub() {
  ...
  return <section className="games-hub">
    ...
    <nav className="game-modules" aria-label="Módulos de juegos"><button className={module === "poo" ? "active" : ""} onClick={() => setModule("poo")}><span className="module-game-icon">◈</span><span><b>POO</b><small>4 pilares · retos interactivos</small></span><strong>→</strong></button><button className={module === "contracts" ? "active" : ""} onClick={() => setModule("contracts")}><span className="module-game-icon contracts">⌘</span><span><b>Interfaces y abstractas</b><small>10 preguntas · comparación</small></span><strong>→</strong></button><button className={module === "interview" ? "active" : ""} onClick={() => setModule("interview")}><span className="module-game-icon interview">⚡</span><span><b>Entrevista POO</b><small>10 preguntas trampa</small></span><strong>→</strong></button><button className={module === "future" ? "active" : ""} onClick={() => setModule("future")}><span className="module-game-icon future">+</span><span><b>Próximamente</b><small>Nuevos juegos y retos</small></span><strong>→</strong></button></nav>
